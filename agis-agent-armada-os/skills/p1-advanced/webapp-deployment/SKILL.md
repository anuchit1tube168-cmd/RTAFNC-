...
2. เลือกปลายทาง: static → GitHub Pages | app มี backend → Lovable/Netlify
3. **BUILD** — deploy จาก main เท่านั้น
4. บันทึก deploy record: SHA + เวลา + URL ลง JobLog ก่อนประกาศ
5. **VERIFY** — เปิด URL จริง: หน้าโหลด + ฟังก์ชันหลัก 1 อย่างทำงาน + ทดสอบบนมือถือ
6. ถ้ามี Apps Script backend → ทดสอบ endpoint ด้วย request จริง 1 ครั้ง
7. พังหลัง deploy → rollback ไป SHA ก่อนหน้าใน 5 นาที แล้วค่อย debug นอกสนาม

## กฎเหล็ก
- ห้าม deploy ก่อนงานสำคัญถ้าไม่มีเวลาเฝ้า
- ทุก deploy ต้องรู้ SHA ที่จะ rollback ไปก่อนกดปุ่ม
- URL production ห้ามชี้ branch ทดลอง

## Fallback
CI/hosting ล่ม → แจกไฟล์ HTML ให้ผู้ใช้เปิด local หรือย้ายขึ้น host สำรอง

## Receipt
live URL + deploy record + screenshot smoke test
